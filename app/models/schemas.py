...
class Inventory(Base):
    ...
    holding_cost_percent = Column(
        Float, default=0.15
    )  # Annual holding cost as % of unit price (e.g., 15%)
    ...
    max_order_qty = Column(
        Integer, nullable=True
    )  # Maximum order quantity (if any limit)

    # ✅ NEW: Inventory Policy
    safety_stock = Column(Integer, default=10)  # Base safety stock level
    reorder_point = Column(
        Integer, nullable=True
    )  # Automatic reorder point (can be calculated)

    # ✅ NEW: Product Metadata
    category = Column(
        String, nullable=True
    )  # Product category (e.g., "Shoes", "Accessories")
...
class AgentCheckpoint(Base):
    ...
    state = Column(
        Text
    )  # Full state JSON (agent_state, progress, decisions, history, errors)
