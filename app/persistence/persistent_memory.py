...
class PersistentMemoryManager:
    ...
    def store_episode(self, db: Session, episode: EpisodicMemory) -> str:
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Stored episodic memory: {episode.event_type} for {episode.sku}"
            )
    ...
    def retrieve_episodes(
        ...
        try:
            query = db.query(schemas.PersistentMemory).filter(
                ...
                schemas.PersistentMemory.timestamp
            ...
            results = (
                query.order_by(schemas.PersistentMemory.timestamp.desc())
                .limit(limit)
                .all()
            )
    ...
    def retrieve_fact(
        ...
        try:
            result = (
                ...
                .order_by(schemas.PersistentMemory.timestamp.desc())
                .first()
            )
    ...
    def retrieve_facts_by_category(
        self, db: Session, category: str, min_confidence: float = 0.0
    ) -> List[SemanticMemory]:
    ...
    def retrieve_procedure(
        self, db: Session, procedure_type: str, name: str
    ...
    def retrieve_best_procedures(
        ...
        try:
            results = (
                ...
                .order_by(schemas.PersistentMemory.confidence.desc())
                .limit(limit)
                .all()
            )
    ...
    def save_checkpoint(self, db: Session, checkpoint: Checkpoint) -> str:
        ...
        try:
            ...
            db.commit()
            logger.info(
                f"Saved checkpoint: {checkpoint.checkpoint_id} for cycle {checkpoint.cycle_number}"
            )
    ...
    def get_latest_stable_checkpoint(
        self, db: Session, goal: Optional[str] = None
    ) -> Optional[Checkpoint]:
    ...
    def get_checkpoint_history(
        self, db: Session, goal: Optional[str] = None, limit: int = 10
    ) -> List[Checkpoint]:
        ...
        try:
            ...
            results = (
                query.order_by(schemas.AgentCheckpoint.timestamp.desc())
                .limit(limit)
                .all()
            )
    ...
    def get_active_goals(self, db: Session) -> List[Goal]:
        ...
        try:
            results = (
                db.query(schemas.PersistentGoal)
                .filter(
                    schemas.PersistentGoal.status == "active",
                    schemas.PersistentGoal.is_active == True,
                )
                .order_by(schemas.PersistentGoal.priority.desc())
                .all()
            )
            ...
            for r in results:
                goals.append(
                    Goal(
                        ...
                        target_metrics=(
                            json.loads(r.target_metrics)
                            if isinstance(r.target_metrics, str)
                            else r.target_metrics
                        ),
                        current_progress=(
                            json.loads(r.current_progress)
                            if isinstance(r.current_progress, str)
                            else r.current_progress
                        ),
                        deadline=r.deadline,
                    )
                )
    ...
    def update_goal_progress(
        self, db: Session, goal_id: str, progress_update: Dict[str, Any]
    ) -> bool:
        """Update progress on a goal"""
        try:
            goal = (
                db.query(schemas.PersistentGoal)
                .filter(schemas.PersistentGoal.goal_id == goal_id)
                .first()
            )
            ...
            current = (
                json.loads(goal.current_progress)
                if isinstance(goal.current_progress, str)
