...
def get_latest_checkpoint(
    ...
    try:
        ...
        if not checkpoint:
            return {"status": "not_found", "message": "No stable checkpoint found"}, 404
...
def get_decision_history_analytics(
    ...
    query = (
        db.query(schemas.AgentMemory, schemas.Feedback.approved, schemas.Feedback.note)
        .outerjoin(
            schemas.Feedback, schemas.Feedback.memory_id == schemas.AgentMemory.id
        )
        .filter(schemas.AgentMemory.created_at >= cutoff_date)
    )
    ...
    for memory, approved, note in results:
        try:
            decision_data = (
                json.loads(memory.decision)
                if isinstance(memory.decision, str)
                else memory.decision
            )
...
def get_learning_progress_analytics(
    ...
    for param in params:
        feedback = (
            db.query(schemas.Feedback)
            .filter(
                schemas.Feedback.sku == param.sku, schemas.Feedback.note.like("[AUTO]%")
            )
            .all()
        )
        ...
        learning_data.append(
            {
                ...
                "accuracy_score": (
                    round(param.accuracy_score, 3) if param.accuracy_score else None
                ),
    ...
    return {
        ...
        "average_approval_rate": round(
            (
                sum(d["approval_rate"] for d in learning_data) / len(learning_data)
                if learning_data
                else 0
            ),
            1,
...
def get_accuracy_by_sku(
    ...
    skus = (
        db.query(schemas.Feedback.sku)
        .distinct()
        .filter(schemas.Feedback.note.like("[AUTO]%"))
        .all()
    )
    ...
    for (sku,) in skus:
        feedback = (
            db.query(schemas.Feedback)
            .filter(schemas.Feedback.sku == sku, schemas.Feedback.note.like("[AUTO]%"))
            .all()
        )
        ...
        accuracy_data.append(
            {
                ...
                "trend": (
                    "improving"
                    if accuracy > 0.75
                    else "stable" if accuracy > 0.5 else "needs_improvement"
                ),
            }
        )
...
def get_improvement_trends_analytics(
    ...
    if len(trends) > 1:
        first_week_avg = sum(t["approval_rate"] for t in trends[:7]) / min(
            7, len(trends)
        )
        last_week_avg = sum(t["approval_rate"] for t in trends[-7:]) / min(
            7, len(trends)
        )
        trend_direction = (
            "improving"
            if last_week_avg > first_week_avg
            else "stable" if abs(last_week_avg - first_week_avg) < 5 else "declining"
        )
    else:
    ...
    return {
        ...
        "average_approval_rate": (
            round(sum(t["approval_rate"] for t in trends) / len(trends), 1)
            if trends
            else 0
        ),
    }
...
def get_memory_summary(
    ...
    auto_feedback = (
        db.query(schemas.Feedback).filter(schemas.Feedback.note.like("[AUTO]%")).count()
    )
    manual_feedback = total_feedback - auto_feedback

    oldest_memory = (
        db.query(schemas.AgentMemory)
        .order_by(schemas.AgentMemory.created_at.asc())
        .first()
    )

    memory_span_days = (
        (datetime.utcnow() - oldest_memory.created_at).days if oldest_memory else 0
    )
    ...
    return {
        ...
        "avg_decisions_per_day": (
            round(total_memories / max(1, memory_span_days), 1)
            if memory_span_days > 0
            else 0
        ),
    }
