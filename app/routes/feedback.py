...
def submit_feedback(
    ...
    mem = (
        ...
        .filter(schemas.AgentMemory.id == payload.memory_id)
        .first()
    )
    if not mem:
...
def get_feedback_history(
    sku: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """
    ...
    try:
        feedback_records = (
            ...
            .filter(schemas.Feedback.sku == sku)
            .order_by(schemas.Feedback.created_at.desc())
            .limit(50)
            .all()
        )
        ...
        if not feedback_records:
            return {
                ...
                "accuracy": 0.5,
                "samples": 0,
                "note": "No feedback yet for this SKU",
            }
            ...
            "parameters": [
                {
                    ...
                    "last_updated": (
                        p.last_updated.isoformat() if p.last_updated else None
                    ),
                }
