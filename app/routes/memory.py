...
def get_memory(
    ...
    rows = (
        ...
        .order_by(schemas.AgentMemory.created_at.desc())
        .limit(limit)
        .all()
    )
    out = []
...
def get_reinforcement_stats(
    ...
    auto_feedback = (
        db.query(schemas.Feedback).filter(schemas.Feedback.note.like("[AUTO]%")).all()
    )
    ...
    for sku in sku_stats:
        sku_stats[sku]["accuracy"] = round(
            (
                (sku_stats[sku]["approved"] / sku_stats[sku]["total"] * 100)
                if sku_stats[sku]["total"] > 0
                else 0
            ),
            2,
        )
    ...
    return {
        ...
        "approval_rate": (
            round(approved_count / total_auto_feedback * 100, 2)
            if total_auto_feedback > 0
            else 0
        ),
