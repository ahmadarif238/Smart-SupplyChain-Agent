...
def get_learning_progress(
    ...
    avg_accuracy = (
        db.query(func.avg(schemas.SKUParameters.accuracy_score)).scalar() or 0.0
    )

    # Get total decisions evaluated (samples_count)
    total_samples = (
        db.query(func.sum(schemas.SKUParameters.samples_count)).scalar() or 0
    )
...
def get_accuracy_by_sku(
    ...
    params = (
        db.query(schemas.SKUParameters)
        .order_by(schemas.SKUParameters.accuracy_score.desc())
        .limit(limit)
        .all()
    )
...
def get_improvement_trends(
    days: int = 7, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """
...
def retrieve_facts(
    ...
    facts = (
        ...
        .filter(schemas.PersistentMemory.memory_type == "semantic")
