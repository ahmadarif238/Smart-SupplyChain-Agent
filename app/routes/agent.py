...
def _execute_agent_cycle(job_id: str):
    ...
    try:
        ...
        log_progress(
            job_id,
            "INIT",
            "🚀 Agent cycle started. Initializing workflow...",
            {"job_id": job_id},
        )
        ...
        log_progress(
            ...
            {"skus_processed": result.get("skus_processed", 0), "status": "completed"},
        )
...
def run_once_async(
    ...
    # Create Job in DB
    new_job = schemas.Job(id=job_id, status="queued", created_at=datetime.utcnow())
    db.add(new_job)
    db.commit()
...
def get_job_status(
    job_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
async def stream_job_progress(
    job_id: str, token: str = None, current_user=None, db: Session = Depends(get_db)
):
    """
    ...
    async def progress_generator():
        ...
        for _ in range(600):  # 10 minutes timeout
            ...
            try:
                job = (
                    scope_db.query(schemas.Job).filter(schemas.Job.id == job_id).first()
                )
                if job:
...
def list_jobs(
    ...
    jobs = (
        db.query(schemas.Job).order_by(schemas.Job.created_at.desc()).limit(limit).all()
    )
...
def get_finance_summary(
    ...
    completed_jobs = (
        db.query(schemas.Job)
        .filter(schemas.Job.status == "completed", schemas.Job.result.isnot(None))
        .order_by(schemas.Job.completed_at.desc())
        .limit(10)
        .all()
    )
    ...
    for order in approved_orders:
        ...
        inventory_item = (
            db.query(schemas.Inventory)
            .filter(schemas.Inventory.sku == order.sku)
            .first()
        )
...
Be concise and executive-focused."""
        ...
        summary_text = call_gemini_api(
            model="gemini-2.5-flash", messages=messages, temperature=0.3, max_tokens=500
        )
