...
class CycleState:
    ...
    overdue_orders: List[Dict[str, Any]] = field(
        default_factory=list
    )  # New: For supplier scoring
    ...
    agent_dialogues: List[Dict[str, Any]] = field(
        default_factory=list
    )  # New: Agent conversations

    # Negotiation tracking (NEW)
    finance_rejections: List[Dict[str, Any]] = field(
        default_factory=list
    )  # Items rejected by Finance
    counter_arguments: List[Dict[str, Any]] = field(
        default_factory=list
    )  # Arguments from Decision
    negotiation_rounds: int = 0
    max_negotiation_rounds: int = 3
    negotiation_proposals: List[Dict[str, Any]] = field(
        default_factory=list
    )  # New: Proposals for Finance to review
    ...
    def is_success(self) -> bool:
        ...
        return (
            len(self.failed_skus) == 0
            or len(self.failed_skus) < len(self.inventory_data) * 0.1
        )
