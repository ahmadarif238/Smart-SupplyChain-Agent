...
class DialogueGenerator:
    ...
    def __init__(self, model: str = None):
        self.model = (
            model or LLMConfig.DIALOGUE_MODEL
        )  # LLMConfig.DIALOGUE_MODEL (gpt-oss-20b)
...
"""
        ...
        try:
            ...
            message = (
                ...
                if response
                else f"Rejecting {product_name} order (${cost:,.2f}). {reason}"
            )
        ...
        return {
            ...
            "sender": agent,
            "receiver": "Decision",
            "content": {"reason": reason, "message": message, "sku": sku, "cost": cost},
            "language": "JSON",
...
"""
        ...
        try:
            ...
            message = (
                response.strip().strip('"')
                if response
                else f"Proposing reconsideration for {product_name}. ROI: {roi:.1f}x"
            )
        except Exception as e:
...
"""
        ...
        try:
            ...
            message = (
                response.strip().strip('"')
                if response
                else f"Approving {product_name} order (${cost:,.2f}). {justification}"
            )
        except Exception as e:
