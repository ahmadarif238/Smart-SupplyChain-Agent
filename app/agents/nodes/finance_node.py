...
class FinanceNode:
    ...
    def review_orders(self, state: CycleState) -> Dict[str, Any]:
        ...
        for decision in reorders:
            ...
            unit_cost = (
                cost_analysis.get("purchasing_cost_per_unit")
                or details.get("unit_price")
                or 10.0
            )
            ...
            is_override = (
                details.get("type") == "threshold_override"
                or details.get("type") == "fallback"
            )
            ...
            if is_override and daily_demand == 0:
                ...
                days_until_stockout = (
                    current_stock / daily_demand if daily_demand > 0 else 999
                )
        ...
        allocation_result = self._solve_budget_allocation(
            scored_decisions, dynamic_budget, state
        )
        ...
        logger.info(
            f"💰 Finance Review: Approved {len(approved_decisions)}, Rejected {len(rejected_decisions)}, Spend ${current_spend:.2f}"
        )
    ...
    def _solve_budget_allocation(
        self, decisions: List[Dict], budget: float, state: CycleState
    ) -> Dict[str, Any]:
        """
        ...
        decision_vars = pulp.LpVariable.dicts(
            "Order", range(len(decisions)), cat="Binary"
        )
        ...
        prob += (
            pulp.lpSum(
                [
                    decisions[i]["finance_metrics"]["projected_value"]
                    * decision_vars[i]
