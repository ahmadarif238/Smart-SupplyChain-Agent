...
class ReviewNode:
    ...
    def review(self, state: CycleState) -> CycleState:
        ...
        """
        logger.info(
            f"🛡️ Review Node: Checking {len(state.decisions)} decisions for risk..."
        )
        ...
        for decision in state.decisions:
            ...
            if total_cost > self.approval_threshold:
                ...
                decision["approval_reason"] = (
                    f"High value order (${total_cost:.2f} > ${self.approval_threshold})"
                )
                approval_queue.append(decision)
                logger.warning(
                    f"✋ Review Node: Flagged {decision['sku']} for approval (Cost: ${total_cost:.2f})"
                )
            else:
