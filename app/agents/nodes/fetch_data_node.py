...
def fetch_data_node(state: CycleState) -> CycleState:
    ...
    try:
        ...
        logger.info(
            f"[{state.cycle_id}] Inventory SKUs: {list(state.inventory_data.keys())}"
        )

        # Fetch recent sales (last 7 days)
        seven_days_ago = datetime.utcnow() - timedelta(days=7)
        sales = (
            db.query(schemas.Sales)
            .filter(schemas.Sales.date >= seven_days_ago)
            .order_by(schemas.Sales.date.desc())
            .all()
        )
        ...
        logger.info(
            f"[{state.cycle_id}] Fetched {len(state.sales_data)} sales records. Revenue (7d): ${total_revenue:.2f}"
        )

        # Fetch recent orders
        orders = (
            db.query(schemas.Orders)
            .order_by(schemas.Orders.order_date.desc())
            .limit(500)
            .all()
        )
        ...
        for order in orders:
            if order.status == "Pending":
                state.pending_orders_by_sku[order.sku] = (
                    state.pending_orders_by_sku.get(order.sku, 0) + order.quantity
                )
                ...
                order_date_naive = (
                    order.order_date.replace(tzinfo=None)
                ...
                if expected_delivery < now:
                    ...
                    logger.warning(
                        f"⚠️ Order #{order.id} for {order.sku} is overdue by {days_overdue} days!"
                    )

        # Fetch recent alerts
        alerts = (
            db.query(schemas.Alerts)
            .order_by(schemas.Alerts.created_at.desc())
            .limit(200)
            .all()
        )
