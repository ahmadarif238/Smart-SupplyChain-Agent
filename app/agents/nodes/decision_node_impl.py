...
class DecisionNode:
    ...
    def _simple_qty_formula(
        self,
        current_qty: int,
        expected_weekly_demand: int,
    ...
    def decide(
        self, sku_item, forecast: Dict[str, Any], learned_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        if learned_params:
            safety_multiplier = learned_params.get(
                "safety_multiplier", self.default_safety_multiplier
            )
        ...
        if forecast_list and len(forecast_list) >= 7:
            ...
            weekly = 0
            if isinstance(forecast, dict):
                text = forecast.get("explanation", "") or forecast.get(
                    "forecast_text", ""
                )
                if text:
        ...
        reorder_required = (current_qty < threshold) or (
            weekly > current_qty and confidence_ok
        )
        qty_to_order = 0
        reason = ""
        learned_note = ""
        ...
        if reorder_required:
            ...
            if learned_params and "accuracy_score" in learned_params:
                ...
                if accuracy > 0.95:
                    learned_note = (
                        f" [High accuracy {accuracy:.0%}: conservative multiplier]"
                    )
                elif accuracy < 0.70:
                    learned_note = (
                        f" [Low accuracy {accuracy:.0%}: aggressive multiplier]"
                    )

            reason = f"Forecasted weekly demand {weekly}, current stock {current_qty}, threshold {threshold}.{learned_note}"
        else:
            reason = (
                f"No reorder: forecasted week {weekly}, current stock {current_qty}."
            )
        ...
        return {
            ...
            "reason": reason,
            "explain": (
                forecast.get("explanation") if isinstance(forecast, dict) else ""
            ),
