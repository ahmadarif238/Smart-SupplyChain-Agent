...
def forecast_node(state: CycleState) -> CycleState:
    ...
    try:
        logger.info(
            f"[{state.cycle_id}] Generating forecasts for {len(state.inventory_data)} SKUs..."
        )
        ...
        def process_sku_forecast(forecast_data):
            ...
            try:
                ...
                if needs_llm and llm_calls_made >= MAX_LLM_CALLS:
                    logger.info(
                        f"LLM call limit reached ({MAX_LLM_CALLS}). Using statistical fallback for {sku}"
                    )
                    return {
                        "sku": sku,
                        "product_name": item.get("product_name"),
                ...
                for attempt in range(2):
                    try:
                        ...
                        if raw is None:
                            logger.warning(
                                f"LLM unavailable for {sku}. Using statistical fallback."
                            )
                            break
                        ...
                        if parsed and isinstance(parsed, dict):
                            ...
                            llm_calls_made += 1
                            logger.info(
                                f"LLM forecast for {sku} (call {llm_calls_made}/{MAX_LLM_CALLS})"
                            )
                            ...
                            logger.warning(
                                f"LLM returned invalid format for {sku}: {type(parsed)}. Using fallback."
                            )
                    except Exception as e:
                        logger.warning(
                            f"LLM forecast attempt {attempt+1} failed for {sku}: {e}"
                        )
                        time.sleep(1)
                ...
                return {
                    ...
                    "product_name": item.get("product_name"),
                    "forecast": stat_forecast
                    or {"forecast": [0] * 7, "confidence": 0, "explanation": "No data"},
                }
        ...
        logger.info(
            f"[{state.cycle_id}] Processing {len(items_to_forecast)} items (max {MAX_LLM_CALLS} LLM calls)"
        )
        ...
        for forecast_data in items_to_forecast:
            ...
            if "error" in result:
                ...
                if isinstance(forecast_dict, dict):
                    ...
                    total_demand = (
                        sum(forecast_list) if isinstance(forecast_list, list) else 0
                    )
                    if total_demand > 100:
                        stream_manager.emit(
                            state.cycle_id,
        ...
        logger.info(
            f"[{state.cycle_id}] Generated {len(state.forecast_results)} forecasts ({llm_calls_made} used LLM)"
        )
        logger.info(
            f"[{state.cycle_id}] Forecast SKUs: {[f['sku'] for f in forecasts[:10]]}..."
        )  # Show first 10
