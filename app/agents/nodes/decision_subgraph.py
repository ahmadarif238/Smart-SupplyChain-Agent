...
from app.agents.nodes.intelligent_decision_node import (
...
def analyze_trends_node(state: CycleState) -> CycleState:
    ...
    """
    logger.info(
        f"[{state.cycle_id}] 📊 Subgraph: Analyzing trends for {len(state.forecast_results)} SKUs..."
    )
    logger.info(
        f"[{state.cycle_id}] 📋 Forecast SKUs: {[f['sku'] for f in state.forecast_results]}"
    )
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(process_metrics, item) for item in state.forecast_results
        ]
...
def check_constraints_node(state: CycleState) -> CycleState:
    ...
    """
    logger.info(
        f"[{state.cycle_id}] 🚧 Subgraph: Checking constraints for {len(state.analyzed_skus)} SKUs..."
    )
    ...
    for item in state.analyzed_skus:
        ...
        if metrics["forecast_confidence"] < _engine.min_confidence_to_order:
            ...
            logger.info(
                f"[{state.cycle_id}] 🔍 {sku}: Low confidence ({metrics['forecast_confidence']:.2f}). Stock={current_stock}, Threshold={threshold}"
            )

            if current_stock < threshold:
                #  Fallback trigger
                fallback_qty = max(
                    metrics["min_order_qty"], int(threshold * 2) - current_stock
                )
                item["constraint_decision"] = "fallback"
                item["fallback_qty"] = fallback_qty
                logger.info(
                    f"[{state.cycle_id}] ✅ {sku}: Fallback triggered. Qty={fallback_qty}"
                )
            else:
                # Hold
                logger.info(
                    f"[{state.cycle_id}] ⏸️  {sku}: Holding (stock >= threshold despite low confidence)"
                )
                continue
        else:
    ...
    logger.info(
        f"[{state.cycle_id}] 🎯 Constraint check complete. {len(constrained_skus)}/{len(state.analyzed_skus)} passed."
    )
...
def optimize_cost_node(state: CycleState) -> CycleState:
    ...
    """
    logger.info(
        f"[{state.cycle_id}] 💎 Subgraph: Optimizing cost for {len(state.constrained_skus)} SKUs..."
    )
    ...
    def process_optimization(item):
        ...
        try:
            ...
            if current_stock < threshold:
                ...
                order_qty = max(
                    metrics_dict.get("min_order_qty", 1), target_stock - current_stock
                )

                logger.info(
                    f"[OVERRIDE] {sku}: Stock {current_stock} < Threshold {threshold} → Ordering {order_qty} units"
                )
            ...
            if item.get("constraint_decision") == "fallback":
                ...
                return {
                    ...
                    "details": {"type": "fallback", "unit_price": metrics.unit_cost},
                    "cost_analysis": {"purchasing_cost_per_unit": metrics.unit_cost},
                    "timestamp": datetime.utcnow().isoformat(),
            ...
            # Normal optimization
            logger.info(
                f"🔍 [Sub-step] {sku}: Analyzing demand trends and constraints..."
            )
            ...
            logger.info(
                f"⚖️ [Sub-step] {sku}: Scoring Urgency & Stockout Risk (Days to Stockout: {days_str})..."
            )
            ...
            urgency = _engine.calculate_urgency(
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [
            executor.submit(process_optimization, item)
            for item in state.constrained_skus
        ]
        for future in as_completed(futures):
            result = future.result()
