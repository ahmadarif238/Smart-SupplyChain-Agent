...
class TokenManager:
    ...
    def can_make_request(self, model: str, estimated_tokens: int) -> bool:
        ...
        with self.lock:
            ...
            tokens_last_minute = sum(
                [tokens for ts, tokens in self.token_usage if current_time - ts < 60]
            )
        ...
        with self.lock:
            ...
            return {
                "requests": {
                    ...
                    "remaining": max(
                        0, LLMConfig.GEMINI_MAX_RPD - requests_last_minute
                    ),
                    "percentage": (
                        requests_last_minute / max(1, LLMConfig.GEMINI_MAX_RPD)
                    )
                    * 100,
                },
                "tokens": {
    ...
    def _check_circuit_breaker(self):
        ...
        for resource in ["requests", "tokens"]:
            if (
                quota[resource]["percentage"]
                > LLMConfig.CIRCUIT_BREAKER_THRESHOLD * 100
            ):
