...
def compress_sales_data(
    sales_records: List[Dict[str, Any]], max_records: int = 30
) -> Dict[str, Any]:
    """
    ...
    try:
        sorted_sales = sorted(
            sales_records, key=lambda x: x.get("date", ""), reverse=True
        )[:max_records]
    except Exception:
...
def compress_negotiation_prompt(
    rejected_item: Dict, inventory_item: Dict, budget_info: Dict
) -> Dict[str, Any]:
    """
    ...
    return {
        ...
        "inventory": {
            ...
            "gap": inventory_item.get("threshold", 10)
            - inventory_item.get("quantity", 0),
        },
        "budget": {
            "remaining": budget_info.get("remaining", 0),
            "shortfall": rejected_item.get("order_value", 0)
            - budget_info.get("remaining", 0),
        },
    }
