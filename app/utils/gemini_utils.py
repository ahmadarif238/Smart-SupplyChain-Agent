...
def query_gemini(
    ...
    if GEMINI_CLIENT is None:
        ...
        raise RuntimeError(
            "Gemini client not installed or GEMINI_API_KEY is not configured."
        )
        ...
        try:
            ...
            if (
                ...
                if retry_count < max_retries:
                    ...
                    retry_count += 1
                    logger.warning(
                        f"Rate limit (429). Retry {retry_count}/{max_retries} after {wait_time}s"
                    )
                    time.sleep(wait_time)
                    continue
                else:
                ...
                logger.error(
                    f"Gemini query failed (attempt {retry_count + 1}): {error_str[:200]}"
                )
                if retry_count < max_retries:
                    retry_count += 1
