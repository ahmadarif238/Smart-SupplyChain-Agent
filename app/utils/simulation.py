...
def simulate_market_activity(db: Session):
    ...
    for item in inventory_items:
        ...
        if random.random() < 0.3:  # 30% chance of activity
            ...
            if item.quantity >= qty_sold:
                ...
                # Record Sale
                sale = schemas.Sales(
                    sku=item.sku, sold_quantity=qty_sold, date=datetime.utcnow()
                )
                db.add(sale)
                ...
                logger.info(
                    f"📉 Sold {qty_sold} of {item.sku}. New Stock: {item.quantity}"
                )
            else:
    ...
    db.commit()
    logger.info(
        f"✅ Simulation complete. {sales_count} sales generated. Revenue: ${total_revenue:.2f}"
    )
