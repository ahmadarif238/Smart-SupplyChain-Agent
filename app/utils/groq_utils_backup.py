        ...
        try:
            ...
            if LLMConfig.ENABLE_TOKEN_TRACKING:
                try:
                    ...
                    usage = resp.usage
                    total_tokens = usage.total_tokens if hasattr(usage, 'total_tokens') else estimated_tokens
                    token_manager.record_request(model, total_tokens)
                    logger.debug(f"✅ LLM call successful: {total_tokens} tokens used ({model})")
            ...
            if "429" in error_str or "rate_limit" in error_str.lower():
                ...
                if retry_count < max_retries:
                    ...
                    quota = token_manager.get_quota_status() if LLMConfig.ENABLE_TOKEN_TRACKING else {}
                    logger.error(
