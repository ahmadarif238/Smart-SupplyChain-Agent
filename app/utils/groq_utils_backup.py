...
# Import token manager and config
...
def query_groq(
    ...
    if GROQ_CLIENT is None:
        raise RuntimeError(
            "groq client not installed or GROQ_API_KEY is not configured."
        )

    # ✅ NEW: Check token budget BEFORE making request
    if LLMConfig.ENABLE_TOKEN_TRACKING:
        estimated_tokens = token_manager.estimate_tokens(prompt, model)

        if not token_manager.can_make_request(model, estimated_tokens):
            quota = token_manager.get_quota_status()
            logger.warning(
                f"⚠️ Token quota check failed for {model}. "
    ...
    while retry_count <= max_retries:
        try:
            ...
            if _rate_limit_delay > 0 and elapsed < _rate_limit_delay:
                ...
                logger.info(
                    f"Rate limit throttle: waiting {wait_time:.1f}s before next Groq call"
                )
            ...
            # ✅ NEW: Extract and record actual token usage
            if LLMConfig.ENABLE_TOKEN_TRACKING:
                try:
                    # Groq returns usage in response object
                    usage = resp.usage
                    total_tokens = (
                        usage.total_tokens
                    ...
                    token_manager.record_request(model, total_tokens)
                    logger.debug(
                        f"✅ LLM call successful: {total_tokens} tokens used ({model})"
                    )
                except Exception as token_err:
                    logger.warning(f"Could not record token usage: {token_err}")
                    # Fallback to estimate
                    token_manager.record_request(model, estimated_tokens)
            ...
            if "429" in error_str or "rate_limit" in error_str.lower():
                ...
                if retry_count < max_retries:
                    ...
                    wait_time = (
                        retry_after if retry_after else (base_delay * (2**retry_count))
                    )
                    ...
                    quota = (
                        token_manager.get_quota_status()
                        if LLMConfig.ENABLE_TOKEN_TRACKING
                        else {}
                    )
                    logger.error(
                        f"Rate limit retries exhausted for {model}. "
                        f"Quota status: {quota}. Error: {error_str[:200]}"
                    )
                ...
                logger.warning(
                    f"Groq query failed (attempt {retry_count + 1}): {error_str[:100]}"
                )
                if retry_count < max_retries:
                    retry_count += 1
